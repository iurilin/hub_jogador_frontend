...
const Profile = () => {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(errorData.erro || "Falha ao atualizar perfil.");
    }
  ...
  const deleteGame = async (id: string) => {
    const token = localStorage.getItem("token");
    if (!token || !confirm("Tem certeza?")) return;
    try {
...
  };
  ...
  if (!user) {
    return <div className="min-h-screen flex items-center justify-center"><p>Não foi possível carregar o perfil.</p></div>;
  }
          ...
          <div className="flex flex-col md:flex-row items-center gap-6 mt-4">
            ...
            <div className="flex-1 text-center md:text-left">
              ...
              <div className="flex flex-wrap gap-3 justify-center md:justify-start">
                {user.posicao && (
                  <Badge className="bg-white/10 hover:bg-white/20 backdrop-blur-sm text-primary-foreground border-white/20">
                    <MapPin className="w-3 h-3 mr-1" /> {user.posicao}
                  </Badge>
                )}
              </div>
            </div>
      ...
      <main className="container mx-auto px-4 py-8">
              ...
              <DialogContent className="max-w-md">
                 ...
                 <div className="space-y-4 py-4">
                    <div className="space-y-2">
                      <Label htmlFor="photo">Foto (Upload futuro)</Label>
                      <Input id="photo" type="file" accept="image/*" onChange={handlePhotoChange} />
                    </div>
                    <div className="space-y-2">
                       <Label htmlFor="name">Nome</Label>
                       <Input id="name" value={editData.nome || ''} onChange={(e) => setEditData({ ...editData, nome: e.target.value })}/>
                    </div>
                    <div className="space-y-2">
        ...
        <div className="mb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <StatsCard title="Gols por Jogo" value={stats.totalJogos > 0 ? (stats.totalGols / stats.totalJogos).toFixed(1) : 0} icon={TrendingUp} />
            </div>
        </div>
        <div className="mb-8">
           <div className="flex items-center justify-between mb-4">
             ...
             <Dialog>
               <DialogTrigger asChild><Button><Plus/> Adicionar Jogo</Button></DialogTrigger>
               <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
                 <DialogHeader><DialogTitle>Registrar Jogo</DialogTitle></DialogHeader>
           ...
           <Card className="shadow-card">
              <CardContent className="p-6">
                {games.length === 0 ? <p>...</p> : games.map(game => (
                   <div key={game._id} className="flex items-center justify-between p-4 rounded-lg border bg-card">
                        <div className="flex-1 mr-4">
                        <p className="font-semibold">{game.adversario}</p>
                        <p className="text-sm text-muted-foreground">
                            {new Date(game.data_hora).toLocaleDateString('pt-BR')} • 
                            Placar: {game.resultado || 'N/A'} • Gols: {game.gols ?? 0}
                        </p>
                    </div>
                      <Button variant="ghost" size="sm" onClick={() => deleteGame(game._id)}><Trash2/></Button>
                   </div>
                ))}
              </CardContent>
           </Card>
        ...
        <div className="mb-8">
           <div className="flex items-center justify-between mb-4">
             <h2 className="text-2xl font-bold">Meus Treinos</h2>
              <Dialog>
               <DialogTrigger asChild><Button><Plus/> Adicionar Treino</Button></DialogTrigger>
               <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
           ...
           <Card className="shadow-card">
             <CardContent className="p-6">
               {trainings.length === 0 ? <p>...</p> : trainings.map(training => (
                  <div key={training._id} className="flex ...">
                    <div className="flex-1 mr-4">
                        <p className="font-semibold">{training.tipo_treino || 'Treino'}</p>
                         <p className="text-sm text-muted-foreground">
                            {new Date(training.data_hora).toLocaleDateString('pt-BR')} • 
                            Duração: {training.duracao_minutos || 'N/A'} min • 
                            Intensidade: {training.intensidade || 'N/A'}
                         </p>
                    </div>
                     <Button variant="ghost" size="sm" onClick={() => deleteTraining(training._id)}><Trash2/></Button>
                  </div>
               ))}
             </CardContent>
           </Card>
